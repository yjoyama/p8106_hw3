...
The metric Accuracy ($\frac{TP+TN}{TP+FP+TN+FN}$) signifies that 89.83% of the samples were correctly classified out of all the samples. Sensitivity ($\frac{TP}{TP+FN}$) indicates that out of all actual Positive Class instances, 94.23% were correctly predicted as `high`. On the other hand, Specificity ($\frac{TN}{FP+TN}$), which is 86.36%, represents the proportion of actual Negative Class instances correctly predicted as `low`.
...
Based on the results, the MARS model exhibits a larger Area Under the Curve (AUC) of `r round(roc.mars$auc[1], 3)`, suggesting enhanced prediction performance compared to logistic regression.
